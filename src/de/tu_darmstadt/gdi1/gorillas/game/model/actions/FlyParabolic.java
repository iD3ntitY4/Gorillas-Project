 ...
 * 
 */
public class FlyParabolic implements Action{
	
	private float angle = 0;
	private int speed = 0;
	
	public FlyParabolic(float angle, int speed)
	{
		this.angle = angle;
		this.speed = speed;
	}
	...
	public void update(GameContainer gc, StateBasedGame sb, int delta, Component event) {
		Vector2f startPos = event.getOwnerEntity().getPosition();
		...
		float newXPos = (float) (startPos.getX() + speedX * (delta*World.DELTA_SCALING) + (0.5* windSpeed*World.WIND_SCALING * Math.pow((delta*World.DELTA_SCALING),2)));
		float newYPos = (float) (startPos.getY() - speedY * (delta*World.DELTA_SCALING) + (0.5* gravAccel * Math.pow((delta*World.DELTA_SCALING),2)));

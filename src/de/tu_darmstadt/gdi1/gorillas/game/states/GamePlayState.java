...
public class GamePlayState extends BasicTWLGameState {
	...
	/**
	 * Is getting called every frame Keeps everything updated
	 */
	@Override
			...
			if (player1Turn) {
				...
				if ((velocityInput1.getText()).matches("\\d+"))
					currentVelocity = Integer
							.parseInt(velocityInput1.getText());
				else
					currentVelocity = -1;
				...
				if ((velocityInput2.getText()).matches("\\d+"))
					currentVelocity = Integer
							.parseInt(velocityInput2.getText());
				else
					currentVelocity = -1;
			}
		}
	...
	protected void layoutRootPane() {
		...
		angleInputLabel1.setPosition(paneWidth / 80, paneHeight / 60);

		angleInput1.setSize(paneWidth / 10, paneHeight / 20);
		angleInput1.setPosition(paneWidth / 80, angleInputLabel1.getY()
				+ angleInput1.getHeight());

		velocityInputLabel1.setSize(paneWidth / 10, paneHeight / 30);
		velocityInputLabel1.setPosition(paneWidth / 80, angleInput1.getY()
				+ paneHeight / 60 + velocityInputLabel1.getHeight());

		velocityInput1.setSize(paneWidth / 10, paneHeight / 20);
		velocityInput1.setPosition(paneWidth / 80, velocityInputLabel1.getY()
				+ velocityInput1.getHeight());
		...
		angleInputLabel2.setPosition(paneWidth - paneWidth / 80
				- angleInputLabel2.getWidth(), paneHeight / 60);
		...
		velocityInputLabel2.setPosition(
				angleInputLabel2.getX(),
				angleInput2.getY() + paneHeight / 60
						+ velocityInputLabel2.getHeight());
		...
		throwButton2.setPosition(angleInputLabel2.getX() - (paneWidth / 40)
				- (throwButton2.getWidth()), paneHeight / 60);
		...
		scoreNameLabel.setPosition(paneWidth / 2
				- (scoreNameLabel.getWidth() / 2), scoreLabel.getY()
				- scoreNameLabel.getHeight());

		scoreContainer.setSize(paneWidth / 6, paneHeight / 8);
		scoreContainer.setPosition(paneWidth / 2
				- (scoreContainer.getWidth() / 2), paneHeight
				- (paneHeight / 30) - (scoreLabel.getHeight() / 2)
				- (scoreNameLabel.getHeight()));

		windLabel.setSize(paneWidth / 6, paneHeight / 16);
		windLabel.setPosition(paneWidth - windLabel.getWidth(), paneHeight
				- (paneHeight / 30) - (windLabel.getHeight() / 2));
		...
		windContainer.setPosition(paneWidth - windContainer.getWidth(),
				paneHeight - (paneHeight / 30) - (windLabel.getHeight() / 2)
						- (windNameLabel.getHeight()));

		endGameContainer.setSize((paneWidth / 3) + paneWidth / 3,
				paneHeight / 3);
		endGameContainer.setPosition(
				paneWidth / 2 - endGameContainer.getWidth() / 2, paneHeight / 2
						- endGameContainer.getHeight() / 2);

		endGameWinnerLabel.setSize(paneWidth / 3 + paneWidth / 3,
				paneHeight / 6);
		endGameWinnerLabel.setPosition(endGameContainer.getX(),
				endGameContainer.getY());

		endGameToMenuButton.setSize(endGameContainer.getWidth() / 3
				+ endGameContainer.getWidth() / 3, paneHeight / 8);
		endGameToMenuButton.setPosition(endGameContainer.getX()
				+ endGameContainer.getWidth() / 6,
				endGameContainer.getY() + endGameContainer.getHeight() / 2
						+ endGameContainer.getHeight() / 12);
	}
	 ...
	 */
	public void initNewGame(DestructibleImageEntity[] skyline,
			Gorilla[] gorillas) {
	...
	public void addListeners() {
		...
		enter.addAction(new EndOfRoundAction(gorillaOne)); // TODO: Change
	...
	public void removeAllEntities() {
		...
		List<Entity> entityList = StateBasedEntityManager.getInstance()
				.getEntitiesByState(Gorillas.GAMEPLAYSTATE);
		for (int i = 0; i < entityList.size(); i++) {
			StateBasedEntityManager.getInstance().removeEntity(
					Gorillas.GAMEPLAYSTATE, entityList.get(i));
		}
	}
	...
	public void endOfTurn() {
		...
		if (player1Turn) {
			tooWeak = bananaCollisionPostion != null
					&& (Math.abs(gorillaOne.getPosition().getX()
							- bananaCollisionPostion.getX()) <= Math
								.abs(gorillaTwo.getPosition().getX()
										- gorillaOne.getPosition().getX()));

			gorillaTwo.sayQuote(entityManager, tooWeak);

		} else {
			tooWeak = bananaCollisionPostion != null
					&& (Math.abs(gorillaTwo.getPosition().getX()
							- bananaCollisionPostion.getX()) <= Math
								.abs(gorillaTwo.getPosition().getX()
										- gorillaOne.getPosition().getX()));
	...
	/**
	 * Gets called at the end of a round (when a gorilla was hit) Increments the
	 * score and checks if the game is over
	 */
	public void endOfRound(Gorilla loser) {
		...
		if (scorePlayer1 >= World.roundsToWin
				|| scorePlayer2 >= World.roundsToWin) {
			endOfGame();
		} else {
			switchTurn();
			this.removeAllEntities();

			DestructibleImageEntity[] skyline = this.createRandomSkyline(sb
					.getContainer().getWidth(), sb.getContainer().getHeight());

			Gorilla[] gorillas = this.placeGorillasRandom(skyline,
					gorillaSizeX, gorillaSizeY);
	...
	/**
	 * Gets called as soon as one player reaches the maximum points Displays the
	 * winner screen and disables throwing actions Adds a new Highscore entry
	 * for both players
	 */
	public void endOfGame() {
		...
		endGameWinnerLabel.setText("CONGRATULATIONS\n\n"
				+ (player1Turn ? World.PLAYER_ONE_NAME.toUpperCase()
						: World.PLAYER_TWO_NAME.toUpperCase()));
		endGameWinnerLabel.setVisible(true);
		endGameToMenuButton.setVisible(true);
				
		if(!debug && sound.getVolume() != 0)
		{
			SoundAnimation noise = new SoundAnimation();
			noise.playSound(SoundAnimation.FANFARE);
		}					
		 ...
		 * playedRounds, scorePlayer1, scorePlayer2, player1Shots,
		 * player2Shots);
		 */

		Highscores.addHighscore(World.PLAYER_ONE_NAME, playedRounds,
				scorePlayer1, player1Shots);
		Highscores.addHighscore(World.PLAYER_TWO_NAME, playedRounds,
				scorePlayer2, player2Shots);

		((HighScoreState) sb.getState(Gorillas.HIGHSCORESTATE))
				.updateHighScoreState();
	}

	/**
	 * Is called as soon as the back to menu button in the end game screen is
	 * pressed Resets end game screen and removes all entites for a new game
	 ...
	 */
	public DestructibleImageEntity[] createRandomSkyline(int frameWidth,
			int frameHeight) {

		BufferedImage image = new BufferedImage(frameWidth / 8,
				frameHeight - 100, BufferedImage.TYPE_INT_RGB);
		...
		for (int i = 0; i < numSkyscrapers; i++) {
			if (!debug) {
				...
				try {
					img[0] = ImageIO.read(new File(skyscraperImagePath
							+ "1.png"));
					img[1] = ImageIO.read(new File(skyscraperImagePath
							+ "2.png"));
					img[2] = ImageIO.read(new File(skyscraperImagePath
							+ "3.png"));
					img[3] = ImageIO.read(new File(skyscraperImagePath
							+ "4.png"));

					graphic.drawImage(img[(int) Math.round(3 * Math.random())],
							0, 0, frameWidth / 8, frameHeight - 100, null);
				} catch (IOException e) {
			...
			skyscrapers[i] = new DestructibleImageEntity("skyscraper" + i,
					image, skyscraperDestructionPath, debug);

			skyscrapers[i].setSize(new Vector2f(frameWidth / numSkyscrapers,
					frameHeight - 100));

			skyscrapers[i].setPosition(new Vector2f(
					(frameWidth / numSkyscrapers) * i
							+ (skyscrapers[i].getSize().x / 2), (skyscrapers[i]
							.getSize().y) / 2 + height));
	 ...
	 *            the height of the current frame
	 * @param yOffsetCity
	 *            the offset of the city to the top of the frame
	 * @param buildingCoordinates
	 *            an ArrayList with Vector positions of the buildings
	 * 
	 * @return an Array with all skyscrapers
	 */
	public DestructibleImageEntity[] createCustomSkyline(int paneWidth,
			int paneHeight, int yOffsetCity,
			ArrayList<Vector2f> buildingCoordinates) {

		BufferedImage image = new BufferedImage(paneWidth / 8, paneHeight
				- yOffsetCity, BufferedImage.TYPE_INT_ARGB);
		Graphics2D graphic = image.createGraphics();
		graphic.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC));
		...
		if (!debug) {
			...
			try {
				...
				graphic.drawImage(img, 0, 0, paneWidth / 8, paneHeight
						- yOffsetCity, null);
			} catch (IOException e) {
		...
		for (int i = 0; i < buildingCoordinates.size(); i++) {
			...
			skyscrapers[i].setSize(new Vector2f(paneWidth
					/ buildingCoordinates.size(), paneHeight - yOffsetCity));

			skyscrapers[i].setPosition(new Vector2f(buildingCoordinates.get(i)
					.getX(), buildingCoordinates.get(i).getY()));
	 ...
	 * 
	 * @param skyscraperArray
	 *            the array of skyscrapers where the gorillas should be placed
	 *            on
	 * @param gorillaWidth
	 ...
	 */
	public Gorilla[] placeGorillasRandom(
			DestructibleImageEntity[] skyscraperArray, int gorillaWidth,
			int gorillaHeight) {
		...
		gorillaOne = new Gorilla(Gorilla.GorillaSide.RIGHT,
				...
				skyscraperArray[g1place].getPosition().y
						...
						- gorillaHeight / 2, gorillaWidth, gorillaHeight, debug);

		int g2place = (int) (8 - 2 * Math.random());
		gorillaTwo = new Gorilla(
				Gorilla.GorillaSide.LEFT,
				(skyscraperArray[g2place].getPosition().x),
				(skyscraperArray[g2place].getPosition().y
						- (skyscraperArray[g2place].getSize().y / 2) - gorillaHeight / 2),
				gorillaWidth, gorillaHeight, debug);
		...
		if (!debug) {
			...
			gorillaOneNameLabel.setPosition(
					(int) gorillaOne.getPosition().x
							- (int) gorillaOne.getSize().x / 2,
					(int) gorillaOne.getPosition().y
							- (int) gorillaOne.getSize().y);

			gorillaTwoNameLabel.setSize((int) gorillaTwo.getSize().x, 32);
			gorillaTwoNameLabel.setPosition(
					(int) gorillaTwo.getPosition().x
							- (int) gorillaOne.getSize().x / 2,
					(int) gorillaTwo.getPosition().y
							- (int) gorillaTwo.getSize().y);
		}
	 ...
	 *            the coordinates for the left gorilla
	 * @param rightGorillaCoordinate
	 *            the coordinates fo the right gorilla
	 * 
	 * @return a 2 element array with both gorillas
	 */
	public Gorilla[] placeGorillasCustom(Vector2f leftGorillaCoordinate,
			Vector2f rightGorillaCoordinate) {
		gorillaOne = new Gorilla(Gorilla.GorillaSide.RIGHT,
				(int) leftGorillaCoordinate.x, (int) leftGorillaCoordinate.y,
				gorillaSizeX, gorillaSizeY, debug);
		gorillaOne = new Gorilla(Gorilla.GorillaSide.LEFT,
				(int) rightGorillaCoordinate.x, (int) rightGorillaCoordinate.y,
				gorillaSizeX, gorillaSizeY, debug);
	...
	public void placeSun() {
		...
		sun = new Sun("Sun", sb.getContainer().getWidth() / 2
				+ sb.getContainer().getWidth() / 8, sb.getContainer()
				.getHeight() / 12, debug);
	...
	/**
	 * Action when the throw button is being pressed Checks the inputs and tells
	 * the gorilla to throw a banana if all inputs are valid
	 */
	public void throwButtonPressed() {
		if (currentAngle > -1 && currentAngle <= 360 && currentVelocity > -1
				&& currentVelocity <= 200 && canThrow) {
			if (player1Turn) {
				gorillaOne.throwBanana(entityManager, currentAngle,
						currentVelocity);
				player1Shots += 1;
			} else {
				gorillaTwo.throwBanana(entityManager, currentAngle,
						currentVelocity);
				player2Shots += 1;
			}
	 ...
	 * 
	 * @param velocity
	 *            the velocity as a single character
	 */
	public void setVelocityInput(char velocity) {
		if (Character.isDigit(velocity)) {
			if (currentVelocity > -1
					&& Integer.parseInt(Integer.toString(currentVelocity) + ""
							+ Character.toString(velocity)) <= 200) {
				currentVelocity = Integer.parseInt(Integer
						.toString(currentVelocity)
						+ ""
						+ Character.toString(velocity));
			} else if (currentVelocity == -1) {
				currentVelocity = Integer
						.parseInt(Character.toString(velocity));
			}
		}
	}
	 ...
	 * 
	 * @param angle
	 *            the angle as a single character
	 */
	public void setAngleInput(char angle) {
		if (Character.isDigit(angle)) {
			if (currentAngle > -1
					&& Integer.parseInt(Integer.toString(currentAngle) + ""
							+ Character.toString(angle)) <= 360) {
				currentAngle = Integer.parseInt(Integer.toString(currentAngle)
						+ "" + Character.toString(angle));
